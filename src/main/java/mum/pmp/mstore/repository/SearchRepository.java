...
public interface SearchRepository extends JpaRepository<Product, Double> {
	...
	List<Product> findProductByNaP(@Param(value = "productName") String productName, @Param(value = "price") double price);
	
	@Query("from Product where productNumber=:productNumber or price=:price")
	List<Product> findProductByNuP(@Param(value = "productNumber") String productNumber, @Param(value = "price") double price);
