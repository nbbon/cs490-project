 ...
 * 
 * @Repository public interface SearchRepository extends JpaRepository<Product,
 * Integer> {
 * 
 * 
 * @Query("from Product where productNumber=:productNumber or productName=:productName or price=:price or category=:category"
 * ) List<Product> findProductBy(@Param(value = "productNumber") String
 * productNumber,
 * 
 * @Param(value = "productName") String productName, @Param(value = "price") int
 * price,
 * 
 ...
 * 
 * 
 * @Query("select * from category") List<Category> getAllCategories();
 * 
